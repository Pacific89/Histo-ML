...
class SuperPatcher():
    """ class for the extraction of SuperPatches. One SuperPtach is a rectangular area around large zones of
        high attention scores (calculated by CLAM)
    """

    def __init__(self, wsi_name, slide_path, mask_path):
        """init method for generating SuperPatches

        Parameters
        ----------
        wsi_name : string
            whole slide image name
        slide_path : string
            path to wsi
        mask_path : string
            path to mask
    ...
    def get_scores(self, blockmap_path):
        """get attention scores from clam generated blockmap paths
    ...
    def redraw_heatmap(self, vis_level=-1):
        """ visualize heatmap and find largest areas of high attentions

        Parameters
        ----------
        vis_level : int, optional
            , by default -1
        """
        ...
        for super_patch in bbox_dict:
            ...
            self.save_patches(bbox, super_patch, downsample)
    ...
    def get_zero_level_region(self, bbox, super_patch, downsample, shift=0):
        """ reads zero level region from whole slide image
        ...
        super_patch : int
            number of current superpatch
        downsample : int
            downsampling factor
        shift : int, optional
            shift applied to bbox, by default 0
        """
    ...
    def export_patches(self, filtered_coords, patch_size, super_patch):
        """ saves patches using multiple threads

        Parameters
        ----------
        filtered_coords : list
            coordinates with attentions higher than treshold
        patch_size : int
            patch size in pixel
        super_patch : int
            number of superpatch
        """
    ...
    def save_patch(self, patch, path):
        """ save patch function to be called in multi threading
        ...
        patch : Pil Image
            WSI patch
        path : string
            path for saving the patch
        """

        patch.save(path + ".png")


    def save_patches(self, bbox, super_patch, downsample):
        """ gets filtered coordinates according to threshold and exports patches

        Parameters
        ----------
        bbox : Dictionary
            dict that holds bounding box info for the superpatch
        super_patch : int
            number for the corresponding superpatch
        downsample : int
            downsampling factor
        """
        patch_size = 256
...
def save_overlay_superpatch(superpatch, files, path, save_path, patch_size=256):
    """ Reads patch overlays generated by HoVerNet, creates and saves an "Overlay-Superpatch"

    Parameters
    ----------
    superpatch : string
        patch to SuperPatch image
    files : list
        list of paths to overlay files
    path : string
        parent path of overlay files
    save_path : string
        path to save overlay superpatch
    patch_size : int, optional
        patch size in pixel of each patch, by default 256
    """
...
def get_overlays(overlay_path):
    """ gets overlays from HoVerNet results
        iterates over "SuperPatches" and calls "save_overlay_superpatch"
        for each found superpatch
    """
    
    save_path = overlay_path.split("/patches")[0]
    ...
    for superpatch in superpatch_nums:
        save_overlay_superpatch(superpatch, files, overlay_path, save_path)

    print(files)


def get_super_patches(wsi, wsi_base_path):
    """ gets blockmap (for attention scores) and the mask file from CLAM
        to create a SuperPatch object

    Parameters
    ----------
    wsi : string
        name of WSI
    wsi_base_path : string
        path to WSI
    """
...
def start_processing(wsi_base_path):
    """ gets all WSI file paths and iterates over each one to extract the "super patches"
    """
...
if __name__ == "__main__":
    ...
    # Create SuperPatches
    wsi_base_path = "/media/user/easystore/public_data/pat_84"
    start_processing(wsi_base_path)

    # Create Overlay SuperPatches (after using HoverNet to create overlays for all single patches)
    overlay_path = "/media/user/easystore/clam_heat_superpatches/DigitalSlide_A1M_10S_1_20190127142442923/patches/overlay/"
    # get_overlays(overlay_path)
